...
{
    if(m_upper.size()>0) { return m_upper[0].size();}
	else { return 0; }
}
// defines the new operator (), so that we can access the elements by A(i,j), index going from i=0,...,dim()-1
...
void band_matrix::lu_decompose()
{
    ...
    for(int k=0; k<this->dim(); k++) {
        ...
        for(int i=k+1; i<=i_max; i++) {
            ...
            for(int j=k+1; j<=j_max; j++) this->operator()(i,j)=this->operator()(i,j)+x*this->operator()(k,j);// assembly part of R
        }
    }
}
// solves Ly=b
...
void spline::set_points(const std::vector<double>& x,  const std::vector<double>& y, bool cubic_spline)
{
    ...
    if(cubic_spline==true) { // cubic spline interpolation
        ...
        // boundary conditions
        if(m_left == spline::second_deriv) {   // 2*b[0] = f''
            A(0,0)=2.0;
            A(0,1)=0.0;
            rhs[0]=m_left_value;
        } else if(m_left == spline::first_deriv) {  // c[0] = f', needs to be re-expressed in terms of b: (2b[0]+b[1])(x[1]-x[0]) = 3 ((y[1]-y[0])/(x[1]-x[0]) - f')
            A(0,0)=2.0*(x[1]-x[0]);
            A(0,1)=1.0*(x[1]-x[0]);
            rhs[0]=3.0*((y[1]-y[0])/(x[1]-x[0])-m_left_value);
        } else { assert(false); }
        if(m_right == spline::second_deriv) { // 2*b[n-1] = f''
            A(n-1,n-1)=2.0;
            A(n-1,n-2)=0.0;
            rhs[n-1]=m_right_value;
        } else if(m_right == spline::first_deriv) {  // c[n-1] = f', needs to be re-expressed in terms of b: (b[n-2]+2b[n-1])(x[n-1]-x[n-2])=3(f'-(y[n-1]-y[n-2])/(x[n-1]-x[n-2]))
    ...
    m_c0 = m_c[0];
    // for the right extrapolation coefficients  f_{n-1}(x) = b*(x-x_{n-1})^2 + c*(x-x_{n-1}) + y_{n-1}
    double h=x[n-1]-x[n-2]; // m_b[n-1] is determined by the boundary condition
    m_a[n-1]=0.0;
...
double spline::operator() (double x) const
{
    ...
    double h=x-m_x[idx];
    double interpol;
    if(x<m_x[0]) {interpol=(m_b0*h + m_c0)*h + m_y[0];} // extrapolation to the left 
	else if(x>m_x[n-1]) { interpol=(m_b[n-1]*h + m_c[n-1])*h + m_y[n-1];} // extrapolation to the right 
	else { interpol=((m_a[idx]*h + m_b[idx])*h + m_c[idx])*h + m_y[idx]; } // interpolation
    return interpol;
}
double spline::deriv(int order, double x) const
{
    ...
    } else if(x>m_x[n-1]) {  // extrapolation to the right
        switch(order) {
